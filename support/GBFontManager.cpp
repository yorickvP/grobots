...
GBFontManager::GBFontManager() : fontlist(), cache(128) {
	if (!TTF_Init()) {
		FatalError("TTF_Init failure: " + string(SDL_GetError()));
	} else {
		atexit(TTF_Quit);
	}
}
...
GBFontManager::GBTTFFont::GBTTFFont(uint8_t ptsize, bool bold) : ptsize(ptsize), bold(bold) {
	...
	if (font == nil) {
		FatalError("TTF_OpenFont failure: " + string(SDL_GetError()));
	}
  this->font = font;
}
...
void GBRenderedText::draw(SDL_Renderer* r, const SDL_FRect* src, const SDL_FRect* dst) {
...
std::shared_ptr<GBRenderedText> GBFontManager::renderText_Blended(SDL_Renderer* renderer, uint8_t ptsize, bool bold, const std::string & text, const GBColor& fgcol) {
  ...
  if (auto v = cache.get(k)) {
    ...
    SDL_Surface* textsurface = TTF_RenderText_Blended(findFont(ptsize, bold), text.c_str(), 0, (SDL_Color)fgcol);
    if (textsurface == nil) FatalError("TTF_RenderText_Blended failure: " + string(SDL_GetError()));
